...
export default function NewListingStep2Screen() {
  ...
  const [activeCategory, setActiveCategory] = useState<CategoryWithSubs | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      unsub = subscribeCategoriesWithSubs((list) => setCategories(list));
    })();
    return () => unsub && unsub();
  }, []);
  ...
  useEffect(() => {
    if (typeof params.itemCondition === "string") {
      setItemCondition(params.itemCondition);
    }
    ...
    if (typeof params.pricingType === "string") {
      setPricingType(params.pricingType);
    }
    if (typeof params.price === "string") {
      setPrice(params.price);
    }
    if (typeof params.buyNowPrice === "string") {
      setBuyNowPrice(params.buyNowPrice);
    }
  }, [
  ...
  const validateForm = () => {
    ...
    const selectedMain = categories.find((c) => toName(c) === category);
    if (selectedMain && selectedMain.subItems?.length && !subcategory) {
      Alert.alert(
        "Subcategory Required",
        "Please select a subcategory for your item.",
      );
      return false;
    }
    if (!price.trim()) {
    ...
    if (pricingType?.includes("Both")) {
      if (!buyNowPrice.trim()) {
        Alert.alert(
          "Buy Now Price Required",
          "Please enter a Buy Now price for your item.",
        );
        return false;
      }
      const numericBuyNow = parseFloat(buyNowPrice);
      if (isNaN(numericBuyNow) || numericBuyNow <= 0) {
        Alert.alert(
          "Invalid Buy Now Price",
          "Please enter a valid Buy Now price greater than 0.",
        );
        return false;
      }
    }
  ...
  const handleNextStep = () => {
    if (validateForm()) {
      const stepData = {
        ...
        pricingType,
        price,
        buyNowPrice: pricingType?.includes("Both") ? buyNowPrice : "",
      };
                ...
                params: {
                  ...
                  description:
                    typeof params.description === "string"
                      ...
                      : "",
                  images:
                    typeof params.images === "string" ? params.images : "[]",
                },
              })
            }
          >
        ...
        >
          ...
          >
            ...
            <View style={styles.card}>
              ...
              <View style={styles.formSection}>
                <Text style={styles.label}>
                  {pricingType?.includes("Auction")
                    ? "Starting Price"
                    : "Price"}{" "}
                  (MUR) <Text style={styles.required}>*</Text>
                </Text>
              ...
              {pricingType?.includes("Both") && (
                <View style={styles.formSection}>
                  <Text style={styles.label}>
                    Buy Now Price (MUR) <Text style={styles.required}>*</Text>
                  </Text>
                  <View style={styles.priceInputContainer}>
                    <Text style={styles.currencySymbol}>Rs</Text>
                    <TextInput
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <ScrollView style={styles.categoryList}>
                ...
                {!activeCategory &&
                  categories.map((cat) => {
                    const name = toName(cat);
                    const hasSubs = !!cat.subItems?.length;
                    const selected =
                      category === name && (!hasSubs || !!subcategory);
                    return (
                      <TouchableOpacity
                        key={cat.id || name}
                        style={[
                ...
                {!!activeCategory &&
                  activeCategory.subItems.map((sub) => {
                    const subName = toName(sub);
                    const selected =
                      category === toName(activeCategory) &&
                      subcategory === subName;
                    return (
                      <TouchableOpacity
                        key={sub.id || subName}
                        ...
                        onPress={() => {
                          setCategory(toName(activeCategory));
