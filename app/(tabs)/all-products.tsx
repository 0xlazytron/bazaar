...
const toCard = (p: Product, type?: ProductType): UICard => {
  ...
  const buyNowPrice = p.pricingType?.includes("Both")
    ? Number(p.price ?? 0)
    : undefined;
...
const isVisibleProduct = (p: Product) =>
  p.status !== "sold" && p.status !== "pending_delivery";
...
export default function AllProductsScreen() {
  ...
  const load = async (filter: FilterType, opts?: { category?: string }) => {
    setLoading(true);
    try {
      if (filter === "All") {
        ...
        setCards(
          list.filter(isVisibleProduct).map((p) => toCard(p, "Featured")),
        );
      } else if (filter === "Ending Soon") {
        const list = await getEndingSoonProducts(30);
        setCards(
          list.filter(isVisibleProduct).map((p) => toCard(p, "Ending Soon")),
        );
      } else if (filter === "Newly Listed") {
        const list = await getNewlyListedProducts(30);
        setCards(
          list.filter(isVisibleProduct).map((p) => toCard(p, "Newly Listed")),
        );
      } else {
        const list = await getPopularProducts(30);
        setCards(
          list.filter(isVisibleProduct).map((p) => toCard(p, "Popular")),
        );
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    if (activeFilter !== "All" && prevFilter === activeFilter) return;

    if (activeFilter === "All") {
      load("All", { category }).catch(() => setCards([]));
      return;
    }
  ...
  useEffect(() => {
    const nextSearch = (params.q as string) || "";
    setSearch(nextSearch);
  }, [params.q]);

  useEffect(() => {
    const nextCategory = (params.category as string) || "";
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchRow}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            load(activeFilter, {
              category: activeFilter === "All" ? category : undefined,
            }).catch(() => setCards([]))
          }
        >
      ...
      <View style={[styles.tabsRow, { paddingBottom: 0 }]}>
        ...
        >
          {filterTabs.map((t) => (
            <TouchableOpacity
              key={t.label}
              style={[styles.tab, activeFilter === t.label && styles.activeTab]}
              onPress={() => setActiveFilter(t.label)}
            >
              <Image
                source={t.icon}
                style={[
                  styles.tabIcon,
                  activeFilter === t.label && styles.activeTabIcon,
                ]}
              />
              <Text
                style={[
                  styles.tabText,
                  activeFilter === t.label && styles.activeTabText,
                ]}
              >
                {t.label}
              </Text>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setShowCat(false)}
      >
