...
export default function NewListingScreen() {
  ...
  useEffect(() => {
    ...
    if (hasDraftData) {
      ...
      if (typeof params.images === "string") {
        try {
          const parsed = JSON.parse(params.images) as Partial<UploadedImage>[];
          const hydrated = Array.isArray(parsed)
            ? parsed
              .map((img) => {
                const id = typeof img?.id === "string" ? img.id : "";
                const downloadURL =
                  typeof img?.downloadURL === "string" ? img.downloadURL : "";
                const uri =
                  typeof img?.uri === "string" ? img.uri : downloadURL || "";
                if (!id || !uri) return null;
                return { id, uri, downloadURL, uploading: false };
              })
              .filter(Boolean)
            : [];
          setImages(hydrated as UploadedImage[]);
        } catch {
          setImages([]);
        }
      } else {
    ...
    if (!params.newListingKey) return;
    setTitle("");
    setDescription("");
    setImages([]);
    setIsUploading(false);
  }, [params.newListingKey, params.title, params.description, params.images]);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      setImages((prev) =>
        prev.map((img) =>
          img.id === imageId ? { ...img, downloadURL, uploading: false } : img,
        ),
      );
    } catch (error) {
      ...
      router.push({
        ...
        params: {
          ...
          images: JSON.stringify(
            images.map((img) => ({ id: img.id, downloadURL: img.downloadURL })),
          ),
        },
      });
    }
