...
export default function ProductDetails() {
  ...
  const formatMemberSince = (value: any) => {
    ...
    const relative =
      years > 0 ? `${years}y${months ? ` ${months}m` : ""}` : `${months}m`;
  ...
  const handleBackPress = () => {
    const key = typeof returnTo === "string" ? returnTo : "";
    if (key === "profile") {
      router.replace({
        pathname: "/(tabs)/profile",
        params: {
          activeTab: typeof returnTab === "string" ? returnTab : "listings",
          scrollTo:
            typeof returnSection === "string" ? returnSection : undefined,
          highlightProductId:
            typeof highlightProductId === "string"
              ? highlightProductId
              : undefined,
          highlightBidId:
            typeof highlightBidId === "string" ? highlightBidId : undefined,
          highlightOrderId:
            typeof highlightOrderId === "string" ? highlightOrderId : undefined,
        },
      } as any);
      return;
    }
    if (key === "all-products") {
      router.replace("/(tabs)/all-products");
      return;
    }
    router.back();
  };
  ...
  useEffect(() => {
    ...
    return subscribeBidHistory(productId, (bids) => {
      setBidHistory(bids);
      setProduct((prev) => {
        if (!prev) return prev;
        if (bids.length === 0) {
          return { ...prev, bidCount: 0 };
        }
        return { ...prev, currentBid: bids[0].amount, bidCount: bids.length };
      });
    });
  }, [id]);
  ...
  useEffect(() => {
    ...
    isProductFavorited(user.uid, productId)
      .then((v) => setIsFavorited(v))
  ...
  const handleShare = async () => {
    ...
    try {
      const title = product.title || product.data || "Product";
      const price =
  ...
  const handlePlaceBid = () => {
    if (!product || product.status !== "active" || timeLeft === "Ended") {
      Alert.alert("Error", "Auction has ended");
      return;
    }
  ...
  const handleReviewBid = () => {
    ...
    if (bidAmountNum <= currentBidNum) {
      Alert.alert(
        "Error",
        `Bid must be higher than current bid of Rs ${currentBidNum}`,
      );
      return;
    }
    ...
    if (bidAmountNum > 10000000) {
      // 1 crore limit
      Alert.alert(
        "Error",
        "Bid amount seems unusually high. Please verify the amount.",
      );
      return;
    }
  ...
  const handleConfirmBid = async () => {
    ...
    try {
      ...
      if (product.status !== "active" || timeLeft === "Ended") {
        Alert.alert("Error", "Auction has ended");
        return;
      }
      ...
      if (error instanceof Error) {
        if (
          ...
          Alert.alert(
            "Permission Error",
            "You do not have permission to place this bid.",
          );
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      setProduct((prev) =>
        prev ? { ...prev, status: "pending_delivery" } : null,
      );
  ...
  const handleLocationPicker = async () => {
    try {
      ...
      if (reverseGeocode.length > 0) {
        ...
        Alert.alert(
          "Location Found",
          "Your current address has been filled in.",
        );
      } else {
        Alert.alert(
          "Location Error",
          "Unable to get address for your current location.",
        );
      }
    } catch (error) {
  ...
  const canBid =
    !!product && product.status === "active" && timeLeft !== "Ended";
  useEffect(() => {
    const updateTime = () => {
      if (!product) {
        setTimeLeft("");
  ...
  const isAuction =
    product.pricingType?.includes("Auction") || product.pricingType === "both";

  // Helper function to format time
  const formatTimeAgo = (timestamp: any) => {
    const now = new Date();
    const bidTime = timestamp?.toDate
      ? timestamp.toDate()
      : new Date(timestamp);
  ...
  return (
    ...
    >
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.statusContainer}>
          <View style={styles.badge}>
            <ThemedText style={styles.badgeText}>
              {product.condition || product.itemCondition || "Used"}
            </ThemedText>
          </View>
            ...
            <>
              ...
              <View style={styles.statusItem}>
                <Image
                  ...
                  style={styles.statusIcon}
                />
                <ThemedText style={styles.statusTextAmber}>
          ...
          {isAuction ? (
            <View>
              ...
              <ThemedText style={styles.currentPrice}>
                Rs {product.currentBid || product.price}
              </ThemedText>
            </View>
          ) : (
            <View>
        ...
        {!isOwner && (
          <View style={styles.bidContainer}>
            ...
            {product.pricingType?.startsWith("Auction") && (
              <>
                <ThemedText style={styles.bidHint}>
                  Enter at least Rs{" "}
                  {Number(product.currentBid || product.price) + 1}
                </ThemedText>
            ...
            {product.status === "active" &&
              (product.pricingType === "Both (Auction with Buy Now option)" ||
                product.pricingType ===
                ...
                <>
                  ...
                  <TouchableOpacity
                    ...
                    disabled={!canBid}
                  >
                    <ThemedText style={styles.placeBidText}>
                      Place Bid
                    </ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      styles.buyNowButton,
                      styles.buyNowButtonWithMargin,
                      !canBid && styles.buyNowButtonDisabled,
                    ]}
                    onPress={handleBuyNow}
                    disabled={!canBid}
                  >
            ...
            {(isAuction || product.pricingType?.includes("Both")) && (
              <View ref={bidHistoryRef} style={styles.bidHistoryContainer}>
                <View style={styles.bidHistoryHeader}>
                  ...
                  />
                  <ThemedText style={styles.bidHistoryTitle}>
                    Bid History
                  </ThemedText>
                </View>
                      ...
                      >
                          ...
                          <View>
                            ...
                            {bid.isHighest && (
                              <View style={styles.highestBidBadge}>
                                <ThemedText style={styles.highestBidText}>
                                  Highest Bid
                                </ThemedText>
                              </View>
                            )}
                        ...
                        <View style={styles.bidAmount}>
                          <Image
                            ...
                            style={[
                              styles.arrowIcon,
                              bid.isHighest && styles.greenArrow,
                            ]}
                          />
                          <ThemedText
                            style={[
                              styles.amountText,
                              bid.isHighest && styles.greenAmount,
                            ]}
                          >
          ...
          <View style={styles.sellerContainer}>
              ...
              <ImageWithLoader
                source={
                  sellerProfile?.photoURL
                    ? { uri: sellerProfile.photoURL }
                    : require("@/assets/images/seller-avatar.jpg")
                }
                style={styles.sellerAvatar}
                loaderSize="small"
                debugLabel="ProductDetails Seller Avatar"
              />
              <View style={styles.sellerInfo}>
                ...
                <View style={styles.ratingContainer}>
                  ...
                  <ThemedText style={styles.ratingText}>
                    4.8/5.0{" "}
                    <ThemedText style={styles.positiveText}>
                      (95% Positive)
                    </ThemedText>
                  </ThemedText>
                </View>
            ...
            <TouchableOpacity
              style={[
                ...
                isOwner && styles.contactButtonDisabled,
              ]}
              disabled={isOwner}
              onPress={() => {
                if (!product || isOwner) return;
                const sellerName =
                  sellerProfile?.displayName || product.sellerName || "Seller";
            ...
            >
              <ThemedText
                style={[
                  ...
                  isOwner && styles.contactButtonTextDisabled,
                ]}
              >
                Contact Seller
              </ThemedText>
      ...
      <BiddingModals
        ...
        productTitle={product.title || "Product"}
        currentBid={
          product.currentBid?.toString() || product.price?.toString() || "0"
        }
