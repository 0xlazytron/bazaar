...
export default function NewListingStep3Screen() {
  ...
  if (typeof params.data === "string") {
    try {
      stepData = JSON.parse(params.data);
    } catch {
      stepData = {};
    }
  }
  ...
  useEffect(() => {
    setDeliveryOption("Both options available");
    setDeliveryCost("");
    setPickupLocation("");
  ...
  }, [params.newListingKey, params.data]);

  useEffect(() => {
    if (deliveryOption === "Pick-up only") {
      setDeliveryCost("");
    }
  }, [deliveryOption]);
  ...
  const validateForm = () => {
    ...
    if (deliveryOption !== "Pick-up only") {
      ...
      if (isNaN(numericDeliveryCost) || numericDeliveryCost < 0) {
        Alert.alert("Error", "Please enter a valid delivery cost");
        return false;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isBothPricing = stepData.pricingType?.includes("Both") || false;
      const isOneDay = stepData.pricingType?.includes("1 day") || false;
      const auctionEndTime = isAuctionType
        ? new Date(now.getTime() + (isOneDay ? 1 : 7) * 24 * 60 * 60 * 1000)
        : null;
      const deliveryCostValue =
        deliveryOption === "Pick-up only" ? 0 : parseFloat(deliveryCost);
      const listingPrice = isBothPricing
        ? stepData.buyNowPrice || ""
        : stepData.price || "";
      ...
      const listingData: any = {
        ...
        images: stepData.images
          ? stepData.images.map((img: any) => img.downloadURL).filter(Boolean)
          : [],
        category: stepData.category || "",
        subcategory: stepData.subcategory || "",
  ...
  const handleBackToStep2 = () => {
    const imagesParam = (() => {
      const images = Array.isArray(stepData?.images) ? stepData.images : [];
      try {
    ...
    router.replace({
      ...
      params: {
        newListingKey:
          typeof params.newListingKey === "string"
            ? params.newListingKey
            : undefined,
        title: typeof stepData?.title === "string" ? stepData.title : "",
        description:
          typeof stepData?.description === "string" ? stepData.description : "",
        images: imagesParam,
        itemCondition:
          typeof stepData?.itemCondition === "string"
            ? stepData.itemCondition
            : "",
        category:
          typeof stepData?.category === "string" ? stepData.category : "",
        subcategory:
          typeof stepData?.subcategory === "string" ? stepData.subcategory : "",
        pricingType:
          typeof stepData?.pricingType === "string" ? stepData.pricingType : "",
        price: typeof stepData?.price === "string" ? stepData.price : "",
        buyNowPrice:
          typeof stepData?.buyNowPrice === "string" ? stepData.buyNowPrice : "",
      },
    });
  };
          ...
          >
            ...
            <View style={styles.titleContainer}>
              ...
              <Text style={styles.subtitle}>
                Let&apos;s help you sell your item quickly
              </Text>
            </View>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.formSection}>
                ...
                <View style={styles.optionsContainer}>
                  {["Delivery", "Pick-up only", "Both options available"].map(
                    (option) => (
                      <TouchableOpacity
                        key={option}
            ...
            <View style={styles.termsModal}>
              ...
              <View style={styles.confirmationList}>
                <Text style={styles.confirmationItem}>
                  • You own this item or are authorized to sell it
                </Text>
                <Text style={styles.confirmationItem}>
                  • The item meets all applicable legal requirements
                </Text>
                <Text style={styles.confirmationItem}>
                  • Your listing complies with Bazaar&apos;s policies
                </Text>
                <Text style={styles.confirmationItem}>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.successModal}>
              ...
              <Text style={styles.successText}>
                Your product has been uploaded and is now live on the
                marketplace. You can view and manage your listings from your
                profile.
              </Text>
              <TouchableOpacity
              ...
              >
                <Text style={styles.viewListingButtonText}>
                  View My Listings
                </Text>
              </TouchableOpacity>
            </View>
